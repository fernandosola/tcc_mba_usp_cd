...
Neste capítulo, serão descritos os recursos de hardware e software utilizados, os experimentos realizados, métricas utilizadas e o modelo escolhido. Por fim, serão exibidos alguns resultados obtidos a partir da utilização do modelo selecionado aplicado em dados reais.

\section{Recursos Utilizados}
\label{section:recursos_utilizados}

Para a realização dos experimentos, utilizou-se um computador com processador Intel(R) Core(TM) i7-7700HQ CPU @ 2.80GHz de 8 núcleos (4 reais, 4 \textit{hyperthreading}), com 32 GB de memória RAM e 500 GB de SSD. O sistema operacional utilizado foi o Windows 10 Home 64 bits. Utilizou-se a distribuição Anaconda (Python 3.7, 64 bits). As principais bibliotecas Python utilizadas são listadas as seguir:
...
\midrule
                 10 &             0,4880 &             0,6166 &   0,7150  \\
                 \textbf{20} &             
                 \textbf{0,5133} &             
                 \textbf{0,6460} &   
                 \textbf{0,7281} \\
                 30 &             0,5108 &             0,6432 &   0,7246  \\
                 40 &             0,5266 &             0,6531 &   0,7366  \\
                 50 &             0,5260 &             0,6471 &   0,7345  \\
                 60 &             0,5263 &             0,6403 &   0,7332  \\
                 70 &             0,5275 &             0,6487 &   0,7354  \\
\bottomrule
\end{tabular}
\legend{Fonte: Autor.}
\end{table}%
...
Em geral, avalia-se que a capacidade distinção entre as classes aprendida pelo modelo é bastante razoável.
...
Após uma análise criteriosa das métricas de cada modelo e das combinações propostas, optou-se por utilizar a combinação de resultados pela maior probabilidade. Ou seja, o decisor otimista. Ele apresenta a maior acurácia balanceada e a segunda maior área abaixo da curva ROC. Além disso, o F1-Score para a classe positiva também teve um bom desempenho em comparação às outras opções. A precisão média não é a melhor, mas a explicação para isto é que escolhe-se aumentar a revocação para a classe 1 em detrimento da sua precisão. Assim, aumenta-se a quantidade de verdadeiros positivos e, por consequência, falsos positivos. Entretanto, diminui-se consideravelmente a quantidade de falsos negativos conforme a premissa adotada para o problema. Mantém-se uma revocação boa e balanceada para ambas as classes.
