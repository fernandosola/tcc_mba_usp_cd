...
\begin{figure}[htbp]
    \caption{Exemplo de extração e expansão de dados estruturados}\label{fig_00200_extracao_expansao}
    \begin{center}
...
Esta etapa consiste na consolidação, em \textit{features}, de todas as informações encontradas nas etapas anteriores. Assim, realiza-se um agrupamento por origem de informações. Algumas características como materialidade são somadas outras como CPF de servidor público são contabilizadas. Dessa forma, para cada denúncia, há um registro com uma quantidade grande de colunas nas quais consolida-se todas as informações encontradas nas etapas anteriores. O quadro XXX apresenta a lista de todas as informações que são extraídas, bem como uma explicação de sua origem e forma de consolidação. Por fim, inclui-se o rótulo.

\textbf{TODO: quadro com todas as features, comentadas, extraídas a partir de dados estruturados}

Com o dataset finalizado podemos iniciar a avaliação dos dados para a proposição de um modelo de classificação.
